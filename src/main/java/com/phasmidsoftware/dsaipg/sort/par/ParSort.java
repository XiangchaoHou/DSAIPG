...
final class ParSort {
    ...
    public static void sort(int[] array, int from, int to, ForkJoinPool myPool) {
        if (to - from >= cutoff) {
            ...
            // TO BE IMPLEMENTED
            completableFuture1 = asyncSort(array, from, (from + to) / 2, myPool);
            completableFuture2 = asyncSort(array, (from + to) / 2, to, myPool);
            // END SOLUTION
     ...
     */
    static int[] sortRecursive(int[] array, int from, int to, ForkJoinPool myPool) {
        int[] result = new int[to - from];
        // TO BE IMPLEMENTED
        result = Arrays.copyOfRange(array, from, to);
        sort(result, 0, result.length, myPool);
        // END SOLUTION
        return result;
    }
    ...
    static CompletableFuture<int[]> asyncSort(int[] array, int from, int to, ForkJoinPool myPool) {
        return CompletableFuture.supplyAsync(
                () -> sortRecursive(array, from, to, myPool),
                myPool
        );
    }
}