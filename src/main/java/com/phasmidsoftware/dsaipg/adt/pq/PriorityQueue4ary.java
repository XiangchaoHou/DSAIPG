...
public class PriorityQueue4ary<K> implements Iterable<K> {
    ...
    public void give(K key) {
        ...
        faryHeap[++last + first - 1] = key; // insert the key into the binary heap just after the last element
    ...
    K doTake(Consumer<Integer> f) {
        K result = faryHeap[first]; // get the root element (the largest or smallest, according to field max)
        ...
        faryHeap[last + first] = null; // prevent loitering
        return result;
    }
    ...
    boolean unordered(int i, int j) {
        return (comparator.compare(faryHeap[i], faryHeap[j]) > 0) ^ max;
    }
    ...
    public Iterator<K> iterator() {
        Collection<K> copy = new ArrayList<>(Arrays.asList(Arrays.copyOf(faryHeap, last + first)));
    ...
    private int parent(int k) {
        if(k < first) System.out.println("k is less than first");
    ...
    private final K[] faryHeap; // faryHeap[i] is ith element of four-ary heap (first element is reserved)
